...
import { getAcademyById, updateAcademyById } from 'apiSdk/academies';
...
function AcademyEditPage() {
  ...
  const handleSubmit = async (values: AcademyInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateAcademyById(id, values);
      mutate(updated);
      resetForm();
    } catch (error) {
      setFormError(error);
  ...
  const formik = useFormik<AcademyInterface>({
    initialValues: data,
